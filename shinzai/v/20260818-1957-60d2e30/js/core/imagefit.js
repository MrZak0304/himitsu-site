...
export function detectHead(imageData, opts = {}) {
  ...
  let widest = 0; let widestY = box.top; let headCenter = null; let headL = 0; let headR = 0;
  for (const r of rows) {
    if (r.y > limit) break;
    if (r.width >= widest) { widest = r.width; widestY = r.y; headCenter = r.mid; headL = r.minX; headR = r.maxX; continue; }
  ...
  return { found: true, top: box.top, neckY, centerX: headCenter, left: headL, right: headR, box };
}
