...
function applyViewMode() {
  ...
  // 単面表示では常に視点操作(◀▶▲▼・ギズモ)を出す。三面のときだけ隠す(第69弾FB)
  $('turnCtl').hidden = mode === 'all';
  const views = document.querySelector('#calcOutput .views');
  if (!views) return;
  views.classList.toggle('single', mode !== 'all');
  // 三面=正面・左側面・右側面の3枚。単面=選んだ面(回している間は turn レンダラー)
  const activeKey = mode === 'all' ? null : (turnActive ? 'turn' : mode);
  views.querySelectorAll('.view').forEach((v, i) => v.classList.toggle('active', mode === 'all' ? VIEW_ORDER[i] !== 'turn' : VIEW_ORDER[i] === activeKey));
}
